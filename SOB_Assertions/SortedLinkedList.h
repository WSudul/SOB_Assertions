...
SortedLinkedList<T>::SortedLinkedList(
    ...
    : comparator_(comparator), size_(0), head_(nullptr), tail_(nullptr) {}

template <typename T> SortedLinkedList<T>::~SortedLinkedList() {
  Clear();
  assert(0 == Size());
}

template <typename T> bool SortedLinkedList<T>::Add(T x) {
  ...
  if (isEmpty()) {
    ...
    if (comparator_(head_->value, new_node->value)) {
      ...
      while (current_node->next) {
        bool comparator_next =
            comparator_(current_node->next->value, new_node->value);
        if (comparator_next)
          break;
...
template <typename T> bool SortedLinkedList<T>::PopFront() {
  if (isEmpty()) {
    return false;
  } else if (head_ == tail_) {
    delete head_;
...
template <typename T> bool SortedLinkedList<T>::PopBack() {
  if (isEmpty()) {
    return false;
  }
...
template <typename T> unsigned long SortedLinkedList<T>::Size() const {
  assert(size_ >= 0);
  return size_;
}

template <typename T> bool SortedLinkedList<T>::isEmpty() const {
  assert(size_ >= 0);
  return (0 == Size());
}

template <typename T> std::string SortedLinkedList<T>::ToString() const {
  if (isEmpty())
    return std::string();
